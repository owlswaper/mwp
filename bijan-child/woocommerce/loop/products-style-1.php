...
?>
<li <?php wc_product_class( $props['special_products'] ? '' : 'cloz-product-card', $product ); ?>>
	<div class="product-inner">
		...
		<div class="product-bottom">
			...
			if( !$props['special_products'] ) {
				if( $options['wishlist'] ) {
					// Keep the parent's nonce and AJAX classes, with native keyboard activation.
					ob_start();
					UI::product_wishlist( $product->get_id() );
					$wishlist_markup = trim( ob_get_clean() );
					$wishlist_markup = str_replace( [ '<div', '</div>' ], [ '<span', '</span>' ], $wishlist_markup );
					$wishlist_markup = '<button type="button" aria-label="علاقه‌مندی‌ها: ' . esc_attr( $product->get_name() ) . '"' . substr( $wishlist_markup, 5 );
					$wishlist_markup = preg_replace( '/<\/span>$/', '</button>', $wishlist_markup );
					echo $wishlist_markup; // Escaped by UI::product_wishlist and esc_attr above.
				}
